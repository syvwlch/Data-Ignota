...
The unique identifier for the actual line of dialog in the original text. 
...
If it is important to count or visualize the data by lines in the original text, the data should be grouped by index first.

There are currently `r said %>% group_by(index) %>% summarize(n = n()) %>% nrow()` lines of dialog in the data but there are `r said %>% nrow()` rows.

#### Index Can Change Between Releases!

The index is assigned dynamically by the Xquery when the file is generated, not embedded in the text as markup. Therefore it can change between releases of the data file.
...
The unique identifier of a character mentioned by name in the line.
